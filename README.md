...
- **청산 시각 표시 방식 변경**: 같은 날짜여도 청산 쪽에 날짜를 생략하지 않고 진입/청산 둘 다 항상 `YYYY-MM-DD HH:MM:SS` 풀로 표시합니다. 며칠씩 들고 있던 거래(수면매매 등)도 헷갈리지 않게.
- 청산 시각이 카드에 안 보이던 문제 수정 (진입 시각만 표시되고 청산 시각은 안 보이던 버그). "거래 정보 수정" 화면에서 진입/청산 시각 직접 수정도 가능.
- 셋업 유형에 '수면매매' 추가
- 분할청산/전량청산 실제 계산 반영: "추가 주문 내역"에서 물타기/추가매수뿐 아니라 분할청산/전량청산/손절도 실제 실현손익 계산에 반영됩니다. 청산 건별 손익이 표시되고, 카드 상단 총 손익(USDT)에도 합산됩니다.
...
service-worker.js       오프라인 캐싱 (캐시 버전 v11 → v12로 올림)
README.md               이 파일
```
...
`service-worker.js`의 캐시 버전을 v11 → v12로 올려뒀어요.
